...
const Report = () => {
  ...
  const getDailyReport = async () => {
    try {
      ...
      const res = await ApiClient.get(
        `/report/attendance?employee_id=${time?.employee_id}&year=${time?.year}&month=${time?.month}&page=1&limit=1`
      );
      setLoader(false);
      if (res.data.data.responseData[0]) {
  ...
  return (
    <div className="min-h-screen py-10">
      <h1 className="text-2xl print:text-xl font-bold mb-2 text-center">
      ...
      <div className="max-w-7xl print:w-[70%] mx-auto p-8 bg-white rounded-lg">
        <div className="flex justify-between items-center mb-8">
          <div className="flex gap-6">
            ...
            <div className="flex gap-2 items-center">
              <h1 className="text-lg font-semibold text-gray-700">Employee:</h1>
              <select
                value={time?.employee_id}
                onChange={(e) =>
                  setTime((prev) => ({ ...prev, employee_id: e.target.value }))
                }
                className="bg-gray-200 text-gray-700 rounded-lg px-4 py-2 focus:outline-none focus:ring focus:ring-blue-300"
              >
                {userArray?.map((userItem, idx) => (
                  <option key={idx} value={userItem?.employee_id}>
                    {userItem?.name}
                  </option>
                ))}
              </select>
          ...
          <div className="p-6 rounded-lg print:shadow-none w-full border print:border-none bg-white relative">
            ...
            <div className="relative z-10">
              <div className="text-center  mb-6">
                <div className="flex px-1 items-center justify-between pt-7">
                  <div>
                    <img
                      src={logo}
                      alt="Company Logo"
                  ...
                  <div>
                    <h2 className="text-lg mb-2 text-end text-gray-600">
        ...
        <div className="flex flex-col items-center mt-8 justify-center ">
          <button
            ...
            className="bg-[#004282] text-white font-bold py-2 px-6 rounded-lg shadow-md hover:bg-[#003366] transition duration-300"
          >
